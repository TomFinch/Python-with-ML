...
def doKMeans(data, clusters=0):
  ...
  #
  user = pd.concat([data.TowerLat, data.TowerLon], axis = 1)
  kmeans = KMeans(n_clusters = clusters)
  #kmeans.fit(user)
  kmeans.fit_predict(user)
...
#
user = user[(user.DOW == 'Mon') | (user.DOW == 'Tue') | (user.DOW == 'Wed') | (user.DOW == 'Thr') | (user.DOW == 'Fri')]

#print (user[(user.TowerLat == [ 32.721944]) & (user.TowerLon == -96.892222)])
#
