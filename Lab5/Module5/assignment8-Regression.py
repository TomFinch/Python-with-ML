...
#
X = pd.read_csv('Datasets/life_expectancy.csv', sep='\t')#, names = ['Year', 'WhiteMale', 'WhiteFemale', 'BlackMale', 'BlackFemale'])

X.WhiteMale = pd.to_numeric(X.WhiteMale, errors='coerce')
X.WhiteFemale = pd.to_numeric(X.WhiteFemale, errors='coerce')
X.BlackMale = pd.to_numeric(X.BlackMale, errors='coerce')
X.BlackFemale = pd.to_numeric(X.BlackFemale, errors='coerce')
#print(X)
#print(X.describe())
...
BlackFemale = model.fit(X_train, y_train)
